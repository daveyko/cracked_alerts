...
function pruneOldTransactions() {
    const now = Date.now();
    console.log("Pruning old transactions. Current cache size:", transactionCache.size);
    transactionCache.forEach((entry, key) => {
        // Filter out old transactions
        entry.transactions = entry.transactions.filter(tx => now - tx.blockTime < TRANSACTION_EXPIRY);
    ...
    console.log("Pruned old transactions. Current cache size:", transactionCache.size);
}
