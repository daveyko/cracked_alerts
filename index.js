                ...
                try {
                    ...
                    for (let i = 0; i < TRANSACTION_NOT_FOUND_RETRIES; i++) {
                        try {
                            ...
                            console.log(
                                `Transaction not found, attempt ${i + 1}/${TRANSACTION_NOT_FOUND_RETRIES}. Waiting ${TRANSACTION_NOT_FOUND_DELAY}ms...`
                            );
                            await new Promise((resolve) =>
                                setTimeout(resolve, TRANSACTION_NOT_FOUND_DELAY)
                            );
                        } catch (error) {
                    ...
                    if (!transaction) {
                        console.log(
                            `Transaction ${signature} not found after ${TRANSACTION_NOT_FOUND_RETRIES} attempts`
                        );
                        return;
                    }
